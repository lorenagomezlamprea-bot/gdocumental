...
export const ApprovedRepository = ({ 
  ...
  const fetchHistory = async (doc: Documento) => {
    ...
    setVersionHistory([]); // Clear previous history
  ...
  const approvedDocs = useMemo(() => {
    return documents.filter(doc => {
      const isApprovedOrReview = doc.estado === 'Aprobado' || doc.estado === 'En revisión';
      ...
      return isApprovedOrReview && matchesProcess && matchesSearch;
    });
